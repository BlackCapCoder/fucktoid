...
data Math = Num Int | Mem Cell | MAdd Math Math | Mult Math Math
...
instance Trans B.Brainfuck Program where
  trans bf | (_, _, _, _, ps) <- foldl parse (True, 0, 0, 0, []) bf
    ...
    where parse (p, s, e, ptr, ps) x = case x of
            ...
            B.Inc    | (Add c v:b) <- ps, c == ptr -> (p, s, e, ptr, Add ptr (MAdd v $ Num 1) : b)
                     | otherwise                  -> (p, s, e, ptr, Add ptr (Num    1) : ps)
            B.Dec    | (Add c v:b) <- ps, c == ptr -> (p, s, e, ptr, Add ptr (MAdd v $ Num 1) : b)
                     | otherwise                  -> (p, s, e, ptr, Add ptr (Num   -1) : ps)
